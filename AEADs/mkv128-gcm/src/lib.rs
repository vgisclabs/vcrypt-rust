...
//!     Mkv128256Gcm, Nonce, Key // Or `Mkv128128Gcm`
//! };
//!
//! # fn gen_key() -> Result<(), core::array::TryFromSliceError> {
//! // The encryption key can be generated randomly:
//! # #[cfg(all(feature = "getrandom", feature = "std"))] {
//! let key = Mkv128256Gcm::generate_key(OsRng);
//! # }
//!
//! // Transformed from a byte array:
//! let key: &[u8; 32] = &[42; 32];
//! let key: &Key<Mkv128256Gcm> = key.into();
//!
...
//! # Ok(()) }
//!
//! # fn main() -> Result<(), mkv128_gcm::Error> {
//! // Alternatively, the key can be transformed directly from a byte slice
//! // (panicks on length mismatch):
//! # let key: &[u8] = &[42; 32];
//! let key = Key::<Mkv128256Gcm>::from_slice(key);
//!
//! let cipher = Mkv128256Gcm::new(&key);
//! let nonce = Mkv128256Gcm::generate_nonce(&mut OsRng); // 96-bits; unique per message
...
//!     Mkv128256Gcm, Nonce, // Or `Mkv128128Gcm`
//! };
//!
//! let key = Mkv128256Gcm::generate_key(&mut OsRng);
//! let cipher = Mkv128256Gcm::new(&key);
//! let nonce = Mkv128256Gcm::generate_nonce(&mut OsRng); // 96-bits; unique per message
//!
...
#[cfg(feature = "mkv128")]
use mkv128::{cipher::consts::U12, Mkv128128, Mkv128256};
...
/// MKV128-GCM with a 128-bit key and 96-bit nonce.
#[cfg(feature = "mkv128")]
#[cfg_attr(docsrs, doc(cfg(feature = "mkv128")))]
pub type Mkv128128Gcm = Mkv128Gcm<Mkv128128, U12>;

/// MKV128-GCM with a 256-bit key and 96-bit nonce.
#[cfg(feature = "mkv128")]
#[cfg_attr(docsrs, doc(cfg(feature = "mkv128")))]
pub type Mkv128256Gcm = Mkv128Gcm<Mkv128256, U12>;

/// MKV128 block.
type Block = GenericArray<u8, U16>;

/// Counter mode with a 32-bit big endian counter.
type Ctr32BE<Mkv128> = ctr::CtrCore<Mkv128, ctr::flavors::Ctr32BE>;

/// MKV128-GCM: generic over an underlying MKV128 implementation and nonce size.
///
/// This type is generic to support substituting alternative MKV128 implementations
/// (e.g. embedded hardware implementations)
///
/// It is NOT intended to be instantiated with any block cipher besides MKV128!
/// Doing so runs the risk of unintended cryptographic properties!
///
/// The `NonceSize` generic parameter can be used to instantiate MKV128-GCM with other
/// nonce sizes, however it's recommended to use it with `typenum::U12`,
/// the default of 96-bits.
///
/// The `TagSize` generic parameter can be used to instantiate MKV128-GCM with other
/// authorization tag sizes, however it's recommended to use it with `typenum::U16`,
/// the default of 128-bits.
///
/// If in doubt, use the built-in [`Mkv128128Gcm`] and [`Mkv128256Gcm`] type aliases.
#[derive(Clone)]
pub struct Mkv128Gcm<Mkv128, NonceSize, TagSize = U16>
where
...
impl<Mkv128, NonceSize, TagSize> KeySizeUser for Mkv128Gcm<Mkv128, NonceSize, TagSize>
where
    Mkv128: KeySizeUser,
    TagSize: self::TagSize,
{
...
where
    Mkv128: BlockSizeUser<BlockSize = U16> + BlockEncrypt + KeyInit,
    TagSize: self::TagSize,
{
...
impl<Mkv128, NonceSize, TagSize> From<Mkv128> for Mkv128Gcm<Mkv128, NonceSize, TagSize>
where
    Mkv128: BlockSizeUser<BlockSize = U16> + BlockEncrypt,
    TagSize: self::TagSize,
{
    fn from(cipher: Mkv128) -> Self {
...
impl<Mkv128, NonceSize, TagSize> AeadCore for Mkv128Gcm<Mkv128, NonceSize, TagSize>
where
...
impl<Mkv128, NonceSize, TagSize> AeadInPlace for Mkv128Gcm<Mkv128, NonceSize, TagSize>
where
    Mkv128: BlockCipher + BlockSizeUser<BlockSize = U16> + BlockEncrypt,
    NonceSize: ArrayLength<u8>,
    TagSize: self::TagSize,
{
    fn encrypt_in_place_detached(
        ...
        // See: <https://github.com/vgisclabs/AEADs/issues/74>
...
impl<Mkv128, NonceSize, TagSize> Mkv128Gcm<Mkv128, NonceSize, TagSize>
where
    Mkv128: BlockCipher + BlockSizeUser<BlockSize = U16> + BlockEncrypt,
    NonceSize: ArrayLength<u8>,
    TagSize: self::TagSize,
{
    ...
    fn init_ctr(&self, nonce: &Nonce<NonceSize>) -> (Ctr32BE<&Mkv128>, Block) {
