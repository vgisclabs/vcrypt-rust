...
/// Result type with the `mkv128-kw` crate's [`Error`].
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Error::InvalidDataSize => write!(f, "data must be a multiple of 64 bits for MKV128-KW and less than 2^32 bytes for MKV128-KWP"),
            Error::InvalidKekSize { size } => {
                write!(f, "invalid MKV128 KEK size: {}", size)
            }
