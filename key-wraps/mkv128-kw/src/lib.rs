...
/// Size of an MKV128 "semiblock" in bytes.
///
...
/// Maximum length of the MKV128-KWP input data (2^32 bytes).
pub const KWP_MAX_LEN: usize = u32::MAX as usize;

/// Size of an MKV128-KW and MKV128-KWP initialization vector in bytes.
pub const IV_LEN: usize = SEMIBLOCK_SIZE;

/// Default Initial Value for MKV128-KW as defined in RFC3394 § 2.2.3.1.
///
...
/// Alternative Initial Value constant prefix for MKV128-KWP as defined in
/// RFC3394 § 3.
///
...
pub struct Kek<Mkv128>
where
    Mkv128: KeyInit + BlockCipher + BlockSizeUser<BlockSize = U16> + BlockEncrypt + BlockDecrypt,
{
    /// Initialized cipher
    cipher: Mkv128,
}
...
/// MKV128-128 KEK
pub type KekMkv128128 = Kek<mkv128::Mkv128128>;

/// MKV128-192 KEK
pub type KekMkv128192 = Kek<mkv128::Mkv128192>;

/// MKV128-256 KEK
pub type KekMkv128256 = Kek<mkv128::Mkv128256>;

impl From<GenericArray<u8, U16>> for KekMkv128128 {
    fn from(kek: GenericArray<u8, U16>) -> Self {
        Kek::new(&kek)
    }
}

impl From<GenericArray<u8, U24>> for KekMkv128192 {
    fn from(kek: GenericArray<u8, U24>) -> Self {
        Kek::new(&kek)
    }
}

impl From<GenericArray<u8, U32>> for KekMkv128256 {
    fn from(kek: GenericArray<u8, U32>) -> Self {
        Kek::new(&kek)
    }
}

impl From<[u8; 16]> for KekMkv128128 {
    fn from(kek: [u8; 16]) -> Self {
        Kek::new(&kek.into())
    }
}

impl From<[u8; 24]> for KekMkv128192 {
    fn from(kek: [u8; 24]) -> Self {
        Kek::new(&kek.into())
    }
}

impl From<[u8; 32]> for KekMkv128256 {
    fn from(kek: [u8; 32]) -> Self {
        Kek::new(&kek.into())
    }
}

impl<Mkv128> TryFrom<&[u8]> for Kek<Mkv128>
where
    Mkv128: KeyInit + BlockCipher + BlockSizeUser<BlockSize = U16> + BlockEncrypt + BlockDecrypt,
{
    type Error = Error;
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        if value.len() == Mkv128::KeySize::to_usize() {
...
impl<Mkv128> Kek<Mkv128>
where
    Mkv128: KeyInit + BlockCipher + BlockSizeUser<BlockSize = U16> + BlockEncrypt + BlockDecrypt,
{
    /// Constructs a new Kek based on the appropriate raw key material.
    pub fn new(key: &GenericArray<u8, Mkv128::KeySize>) -> Self {
        let cipher = Mkv128::new(key);
        Kek { cipher }
    }

    /// MKV128 Key Wrap, as defined in RFC 3394.
    ///
    ...
    /// MKV128 Key Unwrap, as defined in RFC 3394.
    ///
    /// The `out` buffer will be overwritten, and must be exactly [`IV_LEN`]
    ...
    /// MKV128 Key Wrap with Padding, as defined in RFC 5649.
    ///
    ///
    ...
    pub fn wrap_with_padding(&self, data: &[u8], out: &mut [u8]) -> Result<()> {
        ...
        // If n is 1, the plaintext is encrypted as a single MKV128 block
        if n == 1 {
            // 1) Append padding
